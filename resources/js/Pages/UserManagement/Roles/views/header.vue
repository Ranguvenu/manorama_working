    ...
    <div class="flex justify-end items-center w-100">
        <div class="flex items-center justify-items-end w-100">
            <input type="text"
                ...
                placeholder="Search Roles" v-model="filters.search" @input="filter('search', $event.target.value)" />
            <CreateRole :current="current" @close="$emit('close')" @refresh="$emit('refresh')" />
        </div>
    </div>
</template>
<script>
