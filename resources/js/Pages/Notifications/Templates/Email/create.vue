<template>
    <div>
        <Button size="sm" v-if="$has_capability('create-email_template')" v-on:click="open_modal">Add Template</Button>
        <SideModal :show="show" max-width="2xl" @close="close_modal">
            ...
            <div class="mx-2">
                <div v-if="isinitialized">
                    <div class="mt-3">
                        <InputLabel for="notification_type_id">Notification Type <span class="text-red-400">*</span></InputLabel>
                        <MultiSelect class="mt-2" v-model="form.notification_type_id" returns="id" :options="data.notificationtypes" label="title" :multiple="false"/>
                        <InputError class="mt-2" v-if="errors && errors.notification_type_id" :message="errors.notification_type_id[0]"/>
                    </div>
...
export default{
    ...
    methods:{
        ...
        create(){
            let vm = this;
            axios.get('/notifications/template/email/create').then(response => {
        ...
        edit(template){
            let vm = this;
            axios.get('/notifications/template/email/'+template+'/edit').then(response => {
        ...
        store(){
            let vm = this;
            axios.post('/notifications/template/email/store', vm.form).then(response => {
        ...
        update(template){
            let vm = this;
            axios.put('/notifications/template/email/'+template+'/update', vm.form).then(response => {
                if(response.data.success){
                    vm.close_modal();
                    vm.$emit('refresh');
                    this.$toast.success(response.data.message, {                        
