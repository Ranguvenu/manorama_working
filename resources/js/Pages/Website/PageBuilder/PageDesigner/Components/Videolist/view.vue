<template>
    ...
    <div class="w-full my-8">
        ...
        <div class="tab_wrap mb-10 pt-0 pb-8">
            <ul class="flex flex-wrap justify-center text-sm font-medium text-center text-gray-500 dark:text-gray-400 tablist" role="tablist">
                <li class="mr-2 border border-gray-200" role="presentation" v-on:click="filter_postings(0)">
                    <a href="javascript:void(0)" class="inline-block px-8 py-1 bg-white rounded-md text-gray-600 text-md" :class="{'active' : (current == 0)}"
                        id="all" data-tabs-target="#all" type="button" role="tab" aria-controls="all"
                        aria-selected="false">All</a>
                </li>
                <li class="mr-2 border border-gray-200" v-for="(category, index) in options.categories" v-on:click="filter_postings(category.id)">
                    <a href="javascript:void(0)" class="inline-block px-4 py-1 bg-white rounded-md text-gray-600 text-md" :class="{'active' : (current == category.id)}" type="button" role="tab"
        ...
        <div class="grid grid-cols-1 gap-x-6 gap-y-5 mt-3 sm:grid-cols-12" v-if="blogs.data && blogs.data.length">
            <div class="sm:col-span-12 xl:col-span-3 lg:col-span-4 md:col-span-6" v-for="(blog, index) in blogs.data">
                <div class="shadow-[0_4px_14px_rgba(0,0,0,0.04)] rounded-lg w-full">
                    ...
                    <div class="bg-white rounded-bl-lg rounded-br-lg border-l border-r border-b border-zinc-100 px-4 py-6">
                        <div class="break-all text-zinc-800 text-base font-semibold mb-3 min-h-[40px] leading-tight truncate">
                            <!-- <a :title="blog.title" :href="route('webinar.show', {webinar: blog.slug})" v-if="data.type == 'webinar'">{{ blog.title }}</a>
                            <a :title="blog.title" :href="route('blog.show', {category: data.type, article: blog.slug})" v-else-if ="data.type == 'video'">{{ blog.title }}</a>
                            <a :title="blog.title" :href="route('blog.show', {category: data.type, article: blog.slug})" v-else>{{ blog.title }}</a> -->
                        </div>
...
export default {
    ...
    methods: {
        create(){
            let vm = this;
            axios.get(route('website.components.create', {component: 'videolist', type: "video"})).then( (response) => {
                vm.options = response.data;
                console.log('dfdss',response.data);
            }).catch(error => {
        ...
        index(page=1){
            let vm = this;
            axios.post(route('website.components.index', {component: 'videolist', page: page}), vm.filters).then((response) => {
