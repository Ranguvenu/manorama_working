<template>
	...
	<div class="flex md:flex-nowrap flex-wrap items-center imagetext">
		<div class="md:pr-12 mb-2 w-full md:w-1/2" v-if="data && data.description">
			<div class="card-text ck-content" v-html="data.description"></div>
		</div>
		<video-player class="video-player w-full md:!w-1/2 md:mx-auto rounded-[20px] overflow-hidden" v-if="(data && data.media_type == 'video')" :poster="data.image" :src="data.video"
