    ...
    <div>
        <div class="p-3 border">
            <div class="grid grid-cols-1 gap-x-6 gap-y-5 mt-3 sm:grid-cols-12">
                ...
                {{package}}
                <!-- <div v-if="package && package.subjects" v-for="(subject, index) in package.prices">
                    <div class="col-span-6" v-if="subject && subject.title">
                        <InputLabel for="name" :value="subject.title"/>
                    </div>
                    <div v-else class="col-span-6" v-for="(sub, key) in subject">
                        <InputLabel for="name" :value="sub.title" />
                    </div>
                    <TextInput id="name" type="text" class="mt-1 block w-full" v-model="configuration.name[index]"
                    placeholder="name" autofocus v-on:keyup="update_configuration" />
                </div> -->
        ...
        create() {
            ...
            axios.post(route('website.components.create', { component: 'package' }), payload).then((response) => {
                ...
                if (vm.package && vm.package.subjects) {
                    ...
                    console.log('packagesshow:', vm.package);
                }
            }).catch(error => {
