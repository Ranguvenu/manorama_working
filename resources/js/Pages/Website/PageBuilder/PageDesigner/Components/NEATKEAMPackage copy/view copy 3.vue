<template>
    <div v-if="package.is_published == 1">
        ...
        <!-- {{ package.pricing}} -->
        <div class="bg-white" v-for="(pricess, indexs) in package.pricing" >
            <!-- {{pricess}}
            {{indexs}}
            {{package.subjects[pricess]}} -->
            <div class="flex flex-wrap border-zinc-200 border rounded-[15px]">
                <div class="md:w-3/4 custom_md:w-3/5 w-full flex flex-wrap flex-col">
                    ...
                    <div class="bg-pink-50 py-1 px-8">{{package.subjects[0].title}}<span
                            ...
                            <div class="flex flex-row flex-wrap p-8">
                                <div class="md:w-1/2 custom_md:w-full w-full flex items-center mb-5"
                                    v-for="(subject, index) in package.subjects" >
                                    <input type="checkbox" :value="subject.id" :id="'subjectCheckbox' + index" v-model="checkedSubjects[index]" checked
                                        v-on:change="get_price(indexs)" :disabled="!data.is_variable"
                    ...
                    <div class="hidden md:flex justify-between rounded-bl-[11px] px-8 py-5">
                        ...
                        <div class="flex items-center">
                            <a class="text-center text-rose-500 text-sm font-medium leading-[21px] mr-1"
                                href="javascript:void(0)" v-on:click="show_syllabus = !show_syllabus">View Syllabus</a>
                ...
                <div
                    ...
                    <template v-if="subjects && subjects.length">
                        ...
                        <div >
                            <!-- {{package.pricee}} -->
                            <div class="flex items-center mb-1" v-if="package.pricee && package.pricee.actual_price != package.pricee.selling_price">
                                <h1>dsfdskjfnjsdfnjkd</h1>
                                <div class="text-gray-600 text-base line-through leading-[21px] mr-2">₹ {{ package.pricee.actual_price }}
                                </div>
                                <div class="text-green-600 text-base font-semibold mr-2"
                                    v-if="calculate_discount(package.pricee.actual_price, package.pricee.selling_price)">({{
                                        calculate_discount(package.pricee.actual_price, package.pricee.selling_price) }}%)</div>
                            </div>

                            <!-- {{package.pricee}} -->

                            <div class="flex items-center mb-6" v-if="package.pricee && package.pricee.actual_price">

                                <div class="text-zinc-800 text-[32px] font-bold mr-1">₹ {{ package.pricee.selling_price }}/-</div>
    ...
    methods: {
        show() {
            ...
            axios.post(route('website.components.show', { component: 'package' }), payload).then(response => {
                ...
                console.log('subjectssss:', vm.package.subjects)
                console.log('pricing:', vm.package.pricing)
                if (vm.package && vm.package.subjects) {
                    vm.subjects = Array.from(new Set(vm.package.subjects.map(subject => subject.id)));
                    // vm.update_pricing();
                    // const initialIndex = 0;
                    // this.get_price(this.package.pricing[initialIndex]);
                }
            }).catch(error => {
